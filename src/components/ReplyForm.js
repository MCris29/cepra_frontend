...
import { TextField, Button, MenuItem, Stack, Snackbar, AlertTitle } from "@mui/material";
...
const ReplyForm = () => {
    ...
    if(cellEmptyArray.length == 0 && cellUndefinedArray.length == 0) {
      if (surveyReplyArray.length > 0) {
        try {
          ...
          const surveyTemplate = await SurveyTemplates.getById(surveyTemplateId);
          ...
          surveyTemplate.preguntas.forEach(questionTemplate => {
            let questionReply = questionReplyArray.find(
              _questionReply => _questionReply.question == questionTemplate.nombre_pregunta
            );
            ...
            if(questionReply) {
              ...
              questionTemplateNotFoundArray.push(questionTemplate.nombre_pregunta);
            }
          });
          ...
          if(questionTemplateNotFoundArray.length>0 || questionReplyNotFoundArray.length>0) {
            let error = "Error al procesar las preguntas. ";
            if(questionTemplateNotFoundArray.length>0) {
              error += "Preguntas no encontradas: "+questionTemplateNotFoundArray.join(", ")+". ";
            }
            if(questionReplyNotFoundArray.length>0) {
              error += "Preguntas adicionales encontradas: "+questionReplyNotFoundArray.join(", ")+". ";
            }
          ...
          for (let index = 0; index < surveyReplyArray.length; index++) {
            ...
            surveyReply.questionReplyArray.forEach((questionReply) => {
              ...
              let questionTemplate = surveyTemplate.preguntas.find(
                _questionTemplate => questionReply.question == _questionTemplate.nombre_pregunta
              );
              ...
              if(questionTemplate.grupo_opciones != '') {
                ...
                let option = optionArray.find(_option =>
                  _option.itopc_nombre == questionReply.reply
                );
  ...
  const axiosErrorHandler = (error) => {
    ...
    if(error.code === 'ERR_NETWORK') {
      ...
      message = "Error "+error.request.status+"."+" Solicitud "+nameRequest+"fallida.";
      
      message = "Error "+error.request.status+"."+" Solicitud "+nameRequest+"fallida.";
    } else if (error.code === 'ERR_BAD_RESPONSE') {
      message = "Error "+error.response.status+"."+" Respuesta "+nameRequest+"fallida.";
    } else {
    ...
    if (e.target.files.length != 0) {
      ...
      reader.onload = (evt) => {
        try {          
          ...
          for (let index = rowHeaders; index < data[rowHeaders].length; index++) {
            ...
            if(cellData) {
              ...
              if(cellData != "") {
                newQuestionReplyArray.push({question: cellData, questionFind: false});
              } else {
          ...
          if(newCellUndefinedArray.length>0 || newCellEmptyArray.length>0) {
            ...
            if(newCellUndefinedArray.length>0) {
              ...
              error += "Celdas no encontradas: "+newCellUndefinedArray.join(", ")+". ";
            }
          ...
          for (let indexRow = 1; indexRow < data.length; indexRow++) {
            ...
            newQuestionReplyArray.forEach((questionReply, indexCol) => {
              ...
              if(cellData) {
                ...
                questionReplyArray.push({question: cellHeader, reply: cellData});
              } else {
                questionReplyArray.push({question: cellHeader, reply: ""});
              }
            });

            newSurveyReplyArray.push(new SurveyReply(
              organization,
              contact,
              questionReplyArray
            ))
  ...
  return (
    <>
      ...
      <Stack spacing={2} sx={{ width: "100%" }}>
        <Snackbar open={openError} autoHideDuration={6000} onClose={() => {
              setOpenError(false)
            }}>
          <Alert
            onClose={() => {
