...
import {AlertTitle, Button, MenuItem, Select, Snackbar, Stack, TextField} from "@mui/material";
...
const formOrganization = {
    ...
    itorg_ubicacion: {
        label: "Ubicación",
        type: "text",
        defaultValue: "-0.1860160474609731, -78.67458062644613",
        rules: {
            required: "Campo obligatorio!",
        },
    },
...
const ReplyForm = () => {
    ...
    const onSubmitSurvey = async (data) => {
        data = {...data, typeSurveyId: surveyList.find(item => item.itenc_codigo === survey).ittenc_codigo}
        setSurveyForm(data);
    ...
    const onSubmitReply = async (data) => {
        ...
        const surveyQuestions = await SurveyTemplates.getById(
            survey
        );
        ...
        if (data.data) {
            ...
            return (
                <div className="wrapper">
                    <h1>Respuesta individual</h1>
                    {
                        showSurveyForm ? (
                            <form key="formSurvey" onSubmit={handleSubmitSurvey(onSubmitSurvey)}>
                                <h2>Seleccione la encuesta</h2>
                                <section style={{marginBottom: "10px"}}>
                                    ...
                                    />
                                    {errorsSurvey["typeSurvey"] && <span style={{ color: "red"}}>{errorsSurvey["typeSurvey"].message}</span>}
                                </section>
                                <section style={{marginBottom: "10px"}}>
                                    <Controller
                                        name="surveyId"
                                    ...
                                    />
                                    {errorsSurvey["surveyId"] && <span style={{ color: "red"}}>{errorsSurvey["surveyId"].message}</span>}
                                </section>
                                <section style={{marginBottom: "10px"}}>
                                    <Controller
                                        name="fechaTest"
                                    ...
                                    />
                                    {errorsSurvey["fechaTest"] && <span style={{ color: "red"}}>{errorsSurvey["fechaTest"].message}</span>}
                                </section>
                                <br />
                                <hr />
                    ...
                    }
                    {
                        showOrganizationForm ? (
                            <form key="formOrganization" onSubmit={handleSubmitOrganization(onSubmitOrganization)}>
                                <h2>Organización</h2>
                                {
                                    Object.keys(formOrganization).map((e, index) => (
                                        <section key={`org-${index}`} style={{marginBottom: "10px"}}>
                                                ...
                                                />
                                            {errorsOrganization[e] && <span style={{ color: "red"}}>{errorsOrganization[e].message}</span>}
                                            </section>
                                    ))
                                }
                                <h2>Contacto</h2>
                                                                        ...
                                                                        <Controller
                                                                            ...
                                                                            render={({ field }) => (
                                                                                <TextField
                                                                                    ...
                                                                                    value={field.value}
                                                                                    onChange={(event) => field.onChange(event.target.value)}
                                                                                    error={Boolean(errorsReply[question.id.toString()])}
                                                                                />
                                                                            )}
                                                                        />
                                                                        {errorsReply[question.id.toString()] && <span key={`s-${question.id}`} style={{ color: "red"}}>{errorsReply[question.id.toString()].message}</span>}
                                                                    </>
                                                                ) : (
